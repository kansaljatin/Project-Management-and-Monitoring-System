...
function JIITDB() {
  ...
  opDB.getUserByEmail = async function (query) {
    ...
    try {
      ...
      client = new MongoClient(url, { useUnifiedTopology: true });
      await client.connect();
      // console.log("Connecting to JIIT DB...");
      const db = client.db(DB_NAME);
  ...
  opDB.getUserProjects = async function (userId) {
    ...
    try {
      ...
      // console.log("IIIIIIIIIIIIIIIIIIII4");
      const parsedData = await this.getUserByEmail(userId);
  ...
  opDB.getUserProjectCount = async function (userId) {
    ...
    try {
      ...
      // console.log("IIIIIIIIIIIIIIIIIIII3");
      const parsedData = await this.getUserByEmail(userId);
      ...
      if (parsedData.job === 2) {
        ...
        // console.log("got user's project count ", cnt);
        return results;
      } else {
        const results = await projectsCollection.countDocuments({
          ownerId: userId,
        });
        const results2 = await projectsCollection.countDocuments({
          ownerId2: userId,
        });
        const results3 = await projectsCollection.countDocuments({
          ownerId3: userId,
        });
        const results4 = await projectsCollection.countDocuments({
          mentorId: userId,
        });

        const cnt = results + results2 + results3 + results4;
  ...
  // opDB.getAdminProjectCount = async function () {
  //   let client;
  //   try {
  //     console.log("Getting number of projects for admin ...");
  //     client = new MongoClient(url, { useUnifiedTopology: true });
  //     await client.connect();
  //     console.log("Connecting to JIIT DB...");
  //     const db = client.db(DB_NAME);
  ...
  opDB.getPageProjects = async function (userId, page) {
    ...
    try {
      ...
      // console.log("IIIIIIIIIIIIIIIIIIII2");
      const parsedData = await this.getUserByEmail(userId);
  ...
  opDB.searchAndGetProjects = async (query, userId, page) => {
    let client;
    try {
      console.log("YYYYYYYERQWEROQUWEROQWUEROQWUERUQWORUWER");
      console.log(query, userId, page);
  ...
  opDB.updateTaskDetails = async (newTaskObject) => {
    let client;
    try {
      client = new MongoClient(url, { useUnifiedTopology: true });
      await client.connect();
      ...
      const result = await taskCollection.findOneAndUpdate(
        ...
        },
        {
          $set: {
            ...
            taskTitle: newTaskObject.newTitle,
            taskdate: newTaskObject.newdate,
  ...
  opDB.updateFileLocation = async (newTaskObject) => {
    let client;
    try {
      client = new MongoClient(url, { useUnifiedTopology: true });
      await client.connect();
      ...
      const result = await taskCollection.findOneAndUpdate(
        ...
        {
          $set: {
            taskFile: newTaskObject.newFileLocation,
          },
        }
      );
      console.log("updated task");
