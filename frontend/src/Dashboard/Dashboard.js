...
function Dashboard(props) {
  ...
  const newProjectSubmit = async (event) => {
    ...
    const user1 = formData.get("User1");
    const user2 = formData.get("User2");
    const user3 = formData.get("User3");
    const projectName = formData.get("projectName");
    const projectDescription = formData.get("projectDescription");
    var done = false;

    const userDataResult = await fetch(`/userData/username/${user1}`, {
      method: "GET",
    });

    await userDataResult.json().catch((e) => {
      toast.error("Couldn't create the new project. Student 1 doesn't exist");
      done = true;
    });

    if (done) return;

    const userData2Result = await fetch(`/userData/username/${user2}`, {
      method: "GET",
    });

    const parsedUserData2Result = await userData2Result.json().catch((e) => {
      toast.error("Couldn't create the new project. Student 2 doesn't exist");
      done = true;
    });

    if (done) return;

    const userData3Result = await fetch(`/userData/username/${user3}`, {
      method: "GET",
    });

    const parsedUserData3Result = await userData3Result.json().catch((e) => {
      toast.error("Couldn't create the new project. Student 3 doesn't exist");
      done = true;
    });
    ...
    const result = await fetch("/projects", {
      ...
      body: JSON.stringify({
        ...
        mentorId: userData.username,
        ownerId: user1.toLowerCase(),
        ownerId2: user2.toLowerCase(),
        ownerId3: user3.toLowerCase(),
      }),
    });
    if (result) {
      ...
      const dataResult = await fetch(
        `/projects/${userData.username}/page/${page}`,
        {
          method: "GET",
        }
      );
  ...
  useEffect(() => {
    ...
    async function fetchProjectCount() {
      if (loggedInUser && loggedInUser._id) {
        console.log("TESTTT", userData.username);
        const projectCountResult = await fetch(
          `/projects/${userData.username}/count/`,
          {
            method: "GET",
          }
        );
  ...
  useEffect(() => {
    async function getRecentProjects() {
      if (loggedInUser && loggedInUser._id) {
        ...
        const rawData = await fetch(`/projects/${userData.username}/profile`);
              ...
              <div className="modal-dialog">
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    ...
                    >
                      <div className="mb-3">
                        <label htmlFor="User1" className="col-form-label">
                          Student 1:
                        </label>
                        <input
                          type="text"
                      ...
                      <div className="mb-3">
                        <label htmlFor="User2" className="col-form-label">
                          Student 2:
                        </label>
                        <input
                          type="text"
                      ...
                      <div className="mb-3">
                        <label htmlFor="User3" className="col-form-label">
                          Student 3:
                        </label>
                        <input
                          type="text"
