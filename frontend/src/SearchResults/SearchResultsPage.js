...
function SearchResultsPage(props) {
  ...
  useEffect(() => {
    ...
    async function fetchProjectCount() {
      ...
      if (loggedInUser) {
        ...
        const projectCountResult = await fetch(
          `/searchProjects/${userData.username}/count/${query}`,
          {
            method: "GET",
          }
        );
    ...
    fetchProjectCount();
  }, [loggedInUser, query, projectCount, userData]);

  // use effect that pulls projects based on the page that was selected
  useEffect(() => {
    async function fetchProjectData() {
      console.log("PPPPPPPPPPPPPPPPPPPPPPPPPPPP2");
      ...
      if (loggedInUser && loggedInUser._id && query) {
        ...
        const dataResult = await fetch(
          `/searchProjects/${userData.username}/${query}/page/${page}`,
          {
            method: "GET",
          }
        );
    ...
    fetchProjectData();
  }, [loggedInUser, page, query, userData]);
