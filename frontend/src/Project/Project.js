...
const Project = (props) => {
  ...
  const [userDisplayData2, setUserDisplayData2] = useState({});
  const [userDisplayData3, setUserDisplayData3] = useState({});
  const [isUserDataLoading, setIsUserDataLoading] = useState(false);
  const [mentorDisplayData, setMentorDisplayData] = useState({});
  ...
  useEffect(() => {
    ...
    async function fetchProject() {
      if (!projectDeleted) {
        ...
        const mentorDataResult = await fetch(
          `/userData/username/${parsedResult.mentorId}`,
          {
            method: "GET",
          }
        );

        const parsedMentorDataResult = await mentorDataResult.json();
        setMentorDisplayData(parsedMentorDataResult);
        const userDataResult = await fetch(
          `/userData/username/${parsedResult.ownerId}`,
          {
            method: "GET",
          }
        );
        ...
        const parsedUserDataResult = await userDataResult.json();
        setUserDisplayData(parsedUserDataResult);

        const userData2Result = await fetch(
          `/userData/username/${parsedResult.ownerId2}`,
          {
            method: "GET",
          }
        );

        const parsedUserData2Result = await userData2Result.json();
        setUserDisplayData2(parsedUserData2Result);

        const userData3Result = await fetch(
          `/userData/username/${parsedResult.ownerId3}`,
          {
            method: "GET",
          }
        );

        const parsedUserData3Result = await userData3Result.json();
        setUserDisplayData3(parsedUserData3Result);
    ...
    fetchProject();
    fetchTasks();
  }, [projectId, projectDeleted, isUserDataLoading]);
          ...
          <div className="row">
            ...
            >
              <div className="position-sticky pt-3">
                ...
                <h5 className="sidebar-heading-2 d-flex justify-content-between align-items-center px-3 mt-4 mb-1">
                    ...
                    <>
                      <li>
                        <Link
                          ...
                          to={""}
                        >
                          {userDisplayData.fullname} (Leader)
                        </Link>
                      </li>
                      <li>
                        <Link
